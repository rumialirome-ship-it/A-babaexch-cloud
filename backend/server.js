...
async function initializeApp() {
    try {
        ...
        console.log('>>> Dependencies loaded. Initializing services... <<<');
        ...
        app.post('/api/user/bets', authMiddleware, (req, res) => {
            try { const result = database.placeBet(req.user.id, req.body); io.emit('betPlaced', { userId: req.user.id, bet: result }); res.json(result); } catch (e) { res.status(400).json({ message: e.message }); }
        });
        app.post('/api/dealer/bets/bulk', authMiddleware, (req, res) => {
            try {
                ...
                if (user && user.dealerId.toLowerCase() === req.user.id.toLowerCase()) {
                    ...
                    io.emit('betPlaced', { userId: req.body.userId, bet: result });
                    res.json(result);
        ...
        app.post('/api/admin/games/:id/declare-winner', authMiddleware, (req, res) => { 
            ...
            io.emit('winnerDeclared', { gameId: req.params.id, winningNumber: req.body.winningNumber });
            res.json({ success: true }); 
        });
        app.post('/api/admin/games/:id/approve-payouts', authMiddleware, (req, res) => { 
            ...
            io.emit('payoutsApproved', { gameId: req.params.id });
            res.json(result); 
        });
