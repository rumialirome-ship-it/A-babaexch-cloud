...
const calculateBetPayout = (bet: Bet, game: Game | undefined, userPrizeRates: PrizeRates) => {
    ...
    if (!userPrizeRates) return 0;
    const win = game.winningNumber; let count = 0;
    bet.numbers.forEach(num => {
        let isWin = false;
        if (bet.subGameType === SubGameType.OneDigitOpen) isWin = win.length === 2 && num === win[0];
                ...
                {data.map(item => (
                    <div key={item.id} className="bg-slate-800/40 p-4 rounded-2xl border border-slate-700 backdrop-blur-md shadow-lg">
                        <div className="flex justify-between items-start mb-3">
                            <div className="flex items-center gap-2"><img src={item.logo} className="w-8 h-8 rounded-full border border-slate-600" /><div className="text-xs font-black text-white uppercase tracking-tight">{item.name}</div></div>
...
const BetHistoryView: React.FC<{ bets: Bet[], games: Game[], user: User }> = ({ bets, games, user }) => {
    ...
    return (
        <div className="mt-8 space-y-4">
            <div className="flex justify-between items-center"><h3 className="text-xl font-black text-white uppercase tracking-widest">Recent Tickets</h3><input type="date" value={date} onChange={e => setDate(e.target.value)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-1 text-[10px] text-white focus:ring-1 focus:ring-sky-500" /></div>
            <div className="bg-slate-800/40 rounded-2xl overflow-hidden border border-slate-700 shadow-xl overflow-x-auto no-scrollbar">
...
const BettingModal: React.FC<{ game: Game | null, games: Game[], user: User, onClose: () => void, onPlaceBet: (details: any) => Promise<void> }> = ({ game, games, user, onClose, onPlaceBet }) => {
    ...
    const [subType, setSubType] = useState<SubGameType>(SubGameType.TwoDigit);
    ...
    const tabs = useMemo(() => {
        ...
        return game.name === 'AK' ? all.filter(t => t !== SubGameType.OneDigitClose) : all;
    }, [game]);

    useEffect(() => { setManualNums(''); setManualAmt(user?.fixedStake > 0 ? user.fixedStake.toString() : ''); setBulk(''); setError(null); setIsConfirming(false); if (tabs.length > 0 && !tabs.includes(subType)) setSubType(tabs[0]); }, [subType, user?.fixedStake, game, tabs]);
        ...
        try {
            if (subType === SubGameType.Bulk) {
                ...
                lines.forEach(line => {
                    ...
                    tokens.forEach(t => {
                        const type = /^\d[xX]$/i.test(t) ? SubGameType.OneDigitOpen : (/^[xX]\d$/i.test(t) || /^\d$/.test(t) ? SubGameType.OneDigitClose : SubGameType.TwoDigit);
                        const num = type === SubGameType.TwoDigit ? t.padStart(2, '0') : (type === SubGameType.OneDigitOpen ? t[0] : (t.length === 2 ? t[1] : t[0]));
                ...
                const nums = subType === SubGameType.TwoDigit ? (manualNums.replace(/\D/g, '').match(/.{2}/g) || []) : manualNums.replace(/\D/g, '').split('');
                ...
                <div className="p-5 overflow-y-auto no-scrollbar space-y-5">
                    {isConfirming ? (
                        <div className="text-center space-y-6">
                            ...
                            <div className="flex gap-3"><button onClick={() => setIsConfirming(false)} className="flex-1 py-4 bg-slate-800 text-slate-300 font-black uppercase text-[10px] rounded-xl active:scale-95 transition-all">Back</button><button onClick={handleBet} disabled={isSubmitting} className="flex-2 py-4 bg-emerald-600 text-white font-black uppercase text-[10px] rounded-xl shadow-lg active:scale-95 transition-all">{isSubmitting ? 'SYNCING...' : 'CONFIRM & PAY'}</button></div>
                        </div>
                    ) : (
                        <>
                            <div className="flex bg-slate-800 p-1 rounded-xl gap-1 overflow-x-auto no-scrollbar">{tabs.map(t => <button key={t} onClick={() => setSubType(t)} className={`flex-1 min-w-[70px] py-2 text-[8px] font-black uppercase rounded-lg transition-all ${subType === t ? 'bg-slate-700 text-sky-400 shadow-inner' : 'text-slate-500'}`}>{t}</button>)}</div>
                            {subType === SubGameType.Bulk ? (
                                <div><label className="block text-[10px] text-slate-500 font-black uppercase mb-1">Bulk Terminal (Num RS)</label><textarea value={bulk} onChange={e => setBulk(e.target.value)} rows={5} placeholder="Format: 14, 05 50" className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-white font-mono text-sm focus:ring-1 focus:ring-sky-500" /></div>
                            ) : (
                                <>
                                    <div><label className="block text-[10px] text-slate-500 font-black uppercase mb-1">Numbers</label><textarea value={manualNums} onChange={e => setManualNums(e.target.value)} rows={2} placeholder="e.g. 14, 05" className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-white font-mono text-sm focus:ring-1 focus:ring-sky-500" /></div>
                                    <div><label className="block text-[10px] text-slate-500 font-black uppercase mb-1">Stake (Rs)</label><input type="number" value={user?.fixedStake > 0 ? user.fixedStake : manualAmt} onChange={e => setManualAmt(e.target.value)} disabled={user?.fixedStake > 0} className={`w-full p-3 rounded-xl border ${user?.fixedStake > 0 ? 'bg-red-500/10 border-red-500/30 text-red-400' : 'bg-slate-800 border-slate-700 text-white'} font-mono text-sm`} /></div>
                                </>
                            )}
                            ...
                            {error && <div className="bg-red-500/10 border border-red-500/30 text-red-400 text-[9px] p-2 rounded-lg font-bold">{error}</div>}
                            <button onClick={() => { if(finalCost > 0) setIsConfirming(true); }} disabled={finalCost <= 0 || !!error} className="w-full bg-sky-600 hover:bg-sky-500 text-white font-black py-4 rounded-xl text-[10px] uppercase tracking-[0.2em] shadow-xl active:scale-95 transition-all">PROCEED TO BET</button>
                        </>
                    )}
                </div>
            </div>
...
// Component to display financial transactions
            ...
            <div className="overflow-x-auto no-scrollbar">
                <table className="w-full text-left min-w-[700px]">
                    ...
                    <tbody className="divide-y divide-slate-800">
                        ...
                        {entries.length === 0 && (<tr><td colSpan={5} className="p-12 text-center text-slate-500 font-black uppercase text-[10px] tracking-widest">No transaction logs available</td></tr>)}
                    </tbody>
                </table>
            </div>
        </div>
    ...
    return (
        <div className="p-4 sm:p-8 max-w-7xl mx-auto space-y-8 pb-20">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div><h2 className="text-2xl sm:text-4xl font-black text-white uppercase tracking-tighter">My Account</h2><p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">Authorized ID: {user?.id}</p></div>
                <div className="bg-slate-800/60 px-6 py-4 rounded-2xl border border-slate-700 shadow-2xl flex flex-col items-end min-w-[200px]">
                    ...
                    <div className="flex gap-2 mt-2">
                        {user?.fixedStake > 0 && <span className="text-[7px] bg-red-500/20 text-red-400 border border-red-500/20 px-1.5 py-0.5 rounded uppercase font-black">Locked: {user.fixedStake}</span>}
                        <span className="text-[7px] bg-sky-500/20 text-sky-400 border border-sky-500/20 px-1.5 py-0.5 rounded uppercase font-black">Draw Max: {user?.betLimits?.perDraw || 0}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {games.map(g => (
                    <div key={g.id} className={`bg-slate-800/40 p-4 rounded-2xl border border-slate-700 flex flex-col justify-between transition-all ${g.isMarketOpen ? 'hover:border-sky-500/50' : 'opacity-60'}`}>
                        <div className="flex flex-col items-center text-center mb-4">
                            <img src={g.logo} className="w-12 h-12 rounded-full mb-2 border-2 border-slate-700" />
                        ...
                        <div className="bg-black/20 p-2 rounded-xl text-center mb-3">
                            {(g.winningNumber && !g.winningNumber.includes('_')) ? <div className="text-xl font-black text-emerald-400 font-mono">{g.winningNumber}</div> : <div className="text-[10px] font-black text-amber-500 uppercase tracking-widest">{g.isMarketOpen ? 'MARKET OPEN' : 'CLOSED'}</div>}
                        </div>
                        <button onClick={() => setSel(g)} disabled={!g.isMarketOpen || user.isRestricted} className="w-full py-2 bg-sky-600 text-white font-black text-[9px] uppercase tracking-widest rounded-lg active:scale-95 disabled:bg-slate-700 transition-all">Play Now</button>
                    </div>
                ))}
            </div>
