...
const LedgerTable: React.FC<{ entries: LedgerEntry[] }> = ({ entries }) => {
    const sortedEntries = useMemo(() => [...entries].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime()), [entries]);
    return (
    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          ...
          <div className="bg-slate-800/80 p-1 rounded-xl flex items-center space-x-1 border border-slate-700 w-full lg:w-auto overflow-x-auto no-scrollbar shadow-2xl">
            {tabs.map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`shrink-0 flex-1 lg:flex-none flex items-center justify-center space-x-2 py-2 px-5 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-lg transition-all active:scale-95 ${activeTab === tab.id ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}>
      ...
      {activeTab === 'users' && (
        <div className="animate-fade-in space-y-4">
                ...
                {dealerUsers.length === 0 ? (<div className="p-12 text-center text-slate-600 font-black text-xs uppercase">No accounts found</div>) : dealerUsers.map(user => (
                    <div key={user.id} className="p-4 space-y-3">
                        <div className="flex justify-between items-start">
                            <div><div className="font-black text-white text-sm uppercase">{user.name}</div><div className="text-[10px] text-slate-500 font-mono">{user.id}</div>{user.fixedStake > 0 && <div className="text-[8px] bg-red-500/20 text-red-400 px-1 inline-block mt-1">FIXED: Rs {user.fixedStake}</div>}</div>
                            <div className="text-right"><div className="font-mono text-emerald-400 font-black text-sm">Rs {user.wallet.toLocaleString()}</div><div className="text-[8px] text-slate-500 uppercase">Balance</div></div>
                        </div>
                        <div className="flex justify-between items-center text-[10px] font-bold uppercase">
                            <span className="text-slate-400">{user.area || 'No Area'} • {user.commissionRate}% Comm.</span>
                            <button onClick={() => toggleAccountRestriction(user.id, 'user')} className={`px-2 py-0.5 rounded border ${user.isRestricted ? 'bg-rose-500/10 text-rose-500 border-rose-500/20' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'}`}>{user.isRestricted ? 'LOCKED' : 'ACTIVE'}</button>
                        </div>
                        <div className="flex gap-2">
                            <button onClick={() => setViewingUserLedgerFor(user)} className="flex-1 bg-slate-900 border border-slate-700 py-2 rounded-lg text-[9px] font-black text-cyan-400 uppercase active:scale-95 transition-all">Audit Ledger</button>
                            <button onClick={() => { setSelectedUser(user); setIsUserModalOpen(true); }} className="flex-1 bg-slate-900 border border-slate-700 py-2 rounded-lg text-[9px] font-black text-slate-400 uppercase active:scale-95 transition-all">Modify</button>
                        </div>
                    </div>
                ))}
            </div>
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4">
                <button onClick={() => setIsWithdrawalModalOpen(true)} className="bg-slate-800 hover:bg-amber-900/30 text-amber-500 border border-amber-500/30 px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95">Withdraw Funds</button>
                <button onClick={() => setIsTopUpModalOpen(true)} className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl active:scale-95 transition-all">Deposit Funds</button>
          </div>
        </div>
      )}
      ...
      {activeTab === 'tickets' && (
        <div className="animate-fade-in space-y-4">
            ...
            <div className="bg-slate-800/40 rounded-2xl overflow-hidden border border-slate-700 shadow-2xl overflow-x-auto no-scrollbar">
...
const UserForm: React.FC<{ user?: User, onSave: (u: any, o?: string, i?: number) => Promise<void>, onCancel: () => void, dealerPrizeRates: PrizeRates }> = ({ user, onSave, onCancel, dealerPrizeRates }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="pt-4 border-t border-slate-800"><h4 className="text-[10px] font-black text-slate-500 uppercase mb-3 tracking-widest">Risk Management Limits</h4><div className="grid grid-cols-3 gap-2">
            ...
            <div className="flex gap-3 pt-4"><button type="button" onClick={onCancel} className="flex-1 py-3 bg-slate-800 text-slate-300 rounded-xl font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all">Cancel</button><button type="submit" disabled={loading} className="flex-2 py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg active:scale-95 transition-all">{loading ? 'SYNCING...' : 'COMMIT'}</button></div>
        </form>
    );
};
...
const UserTransactionForm: React.FC<{ type: string, users: User[], onTransaction: (u: string, a: number) => Promise<void>, onCancel: () => void }> = ({ type, users, onTransaction, onCancel }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2"><button type="button" onClick={onCancel} className="flex-1 py-3 bg-slate-800 text-slate-300 rounded-xl font-black text-[10px] uppercase active:scale-95 transition-all">Abort</button><button type="submit" disabled={loading || !userId || amount <= 0} className={`flex-2 py-3 ${type === 'Withdrawal' ? 'bg-amber-600' : 'bg-emerald-600'} text-white rounded-xl font-black text-[10px] uppercase shadow-xl active:scale-95 transition-all disabled:opacity-50`}>{loading ? 'PROCESSING...' : `EXECUTE ${type}`}</button></div>
        </form>
    );
};
...
const BettingTerminalView: React.FC<{ users: User[]; games: Game[]; placeBetAsDealer: (details: any) => Promise<void> }> = ({ users, games, placeBetAsDealer }) => {
    ...
    const handleProcessBets = async () => {
        ...
        try {
            ...
            lines.forEach(line => {
                ...
                const isCombo = /\b(k|combo)\b/i.test(betPart); betPart = betPart.replace(/\b(k|combo)\b/i, '').trim();
                ...
                tokens.forEach(token => {
                    if (isCombo) {
                        ...
                        const isOneOpen = /^\d[xX]$/i.test(token); const isOneClose = /^[xX]\d$/i.test(token) || /^[xX]?\d$/.test(token) && games.find(g => g.id === selectedGameId)?.name === 'AKC';
                        const isTwo = /^\d{1,2}$/.test(token);
                        const type = isOneOpen ? SubGameType.OneDigitOpen : (isOneClose ? SubGameType.OneDigitClose : (isTwo ? SubGameType.TwoDigit : null));
                        if (type) {
                            let val = token; if (type === SubGameType.TwoDigit) val = token.padStart(2, '0'); else if (type === SubGameType.OneDigitOpen) val = token[0]; else val = token.length === 2 ? token[1] : token[0];
            ...
            setBulkInput(''); alert("Terminal Batch Confirmed");
    ...
    return (
        <div className="bg-slate-800/50 p-4 sm:p-6 rounded-2xl border border-slate-700 shadow-2xl space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <select value={selectedUserId} onChange={e => setSelectedUserId(e.target.value)} className="bg-slate-900 text-white p-3 rounded-xl border border-slate-700 text-[10px] font-black uppercase active:scale-95 transition-transform"><option value="">-- Player Account --</option>{users.map(u => <option key={u.id} value={u.id}>{u.name} ({u.id})</option>)}</select>
                <select value={selectedGameId} onChange={e => setSelectedGameId(e.target.value)} className="bg-slate-900 text-white p-3 rounded-xl border border-slate-700 text-[10px] font-black uppercase active:scale-95 transition-transform"><option value="">-- Market --</option>{games.filter(g => g.isMarketOpen).map(g => <option key={g.id} value={g.id}>{g.name}</option>)}</select>
            </div>
            <textarea rows={8} value={bulkInput} onChange={e => setBulkInput(e.target.value)} placeholder="Entry Format Example:&#10;14, 25 50&#10;x4, 9x 100&#10;k123 20" className="w-full bg-slate-900 text-white p-4 rounded-xl border border-slate-700 font-mono text-sm no-scrollbar" />
