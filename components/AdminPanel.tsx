        ...
        <div className="bg-slate-800/40 rounded-2xl border border-slate-700 overflow-hidden flex flex-col h-full shadow-2xl">
            <div className="p-4 border-b border-slate-700 bg-slate-800/80 flex justify-between items-center">
                ...
                <button 
                    ...
                    className="text-[9px] font-black text-cyan-400 uppercase tracking-tighter hover:text-white transition-colors flex items-center gap-1 bg-cyan-500/10 px-2 py-1 rounded disabled:opacity-30 active:scale-90"
                >
            ...
            <div className="flex-grow overflow-y-auto max-h-[500px] no-scrollbar">
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-800/50">
                        {stakes.length === 0 ? (
                            <tr><td colSpan={2} className="p-10 text-center text-slate-600 text-[10px] uppercase font-black">No Data Logged</td></tr>
                        ) : stakes.map(s => (
        ...
        <div className="animate-fade-in space-y-6">
            <div className="flex flex-col sm:flex-row gap-4 bg-slate-800/40 p-4 rounded-2xl border border-slate-700">
                <div className="flex-1">
                    <div className="flex justify-between items-end mb-1">
                        <label className="text-[10px] text-slate-500 font-black uppercase tracking-widest block">Draw Date</label>
                        <button onClick={() => setDate('')} className="text-[8px] font-black text-red-400 uppercase tracking-widest hover:underline active:opacity-50">Show All Time</button>
                    </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <SummaryTable title="2-Digit Summary" stakes={data?.twoDigit || []} />
                    <SummaryTable title="1-Digit Open" stakes={data?.oneDigitOpen || []} />
                    <SummaryTable title="1-Digit Close" stakes={data?.oneDigitClose || []} />
                </div>
            )}
        </div>
    );
...
const DealerTransactionForm: React.FC<{ 
    ...
    return (
        <form onSubmit={async (e) => { e.preventDefault(); if (selectedId && amount && amount > 0) { await onTransaction(selectedId, Number(amount)); } }} className="space-y-4">
            ...
            <div className="flex gap-3 pt-4">
                <button type="button" onClick={onCancel} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white font-bold py-3 rounded-xl text-xs uppercase tracking-widest transition-all active:scale-95">Cancel</button>
                <button type="submit" className={`flex-1 font-black py-3 rounded-xl text-white text-xs shadow-lg bg-${themeColor}-600 hover:bg-${themeColor}-500 transition-all uppercase tracking-widest active:scale-95 active:bg-${themeColor}-700`}>Proceed {type}</button>
            </div>
        </form>
    );
};
    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="bg-slate-800/80 p-1 rounded-xl flex items-center space-x-1 border border-slate-700 w-full sm:w-auto overflow-x-auto no-scrollbar">
            {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`shrink-0 flex items-center space-x-2 py-2 px-4 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-lg transition-all active:scale-95 ${activeTab === tab.id ? 'bg-red-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}>
                     ...
                     <tbody className="divide-y divide-slate-800">
                         {dealers.map(dealer => (
                             <tr key={dealer.id} className="hover:bg-slate-700/20 transition-all">
                                 ...
                                 <td className="p-4 text-center">
                                     <button onClick={() => toggleAccountRestriction(dealer.id, 'dealer')} className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-tighter transition-all active:scale-90 ${dealer.isRestricted ? 'bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500/20' : 'bg-green-500/10 text-green-500 border border-green-500/20 hover:bg-green-500/20'}`}>
                                         {dealer.isRestricted ? 'Restricted' : 'Operational'}
                                     </button>
                                 </td>
                                 <td className="p-4">
                                    <div className="flex justify-center gap-2">
                                        <button onClick={() => { setActionDealerId(dealer.id); setIsWithdrawModalOpen(true); }} className="bg-amber-600/10 text-amber-500 border border-amber-500/30 px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest hover:bg-amber-600/20 transition-all active:scale-90">Withdraw</button>
                                        <button onClick={() => { setActionDealerId(dealer.id); setIsDepositModalOpen(true); }} className="bg-emerald-600/10 text-emerald-500 border border-emerald-500/30 px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest hover:bg-emerald-600/20 transition-all active:scale-90">Deposit</button>
                                    </div>
                                 </td>
                                 <td className="p-4 text-right flex justify-end gap-3">
                                    <button onClick={() => { setViewingLedgerId(dealer.id); setViewingLedgerType('dealer'); }} className="text-[10px] font-black text-cyan-400 uppercase tracking-widest hover:underline hover:text-cyan-300 active:opacity-50 transition-opacity">Audit Ledger</button>
                                    <button onClick={() => { setSelectedDealer(dealer); setIsDealerModalOpen(true); }} className="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-white transition-all active:scale-90">Edit Profile</button>
                                 </td>
                             </tr>
                         ))}
                         {dealers.length === 0 && (
                        ...
                        <tbody className="divide-y divide-slate-800">
                            {users.map(user => (
                                <tr key={user.id} className="hover:bg-slate-700/20 transition-all">
                                    ...
                                    <td className="p-4 text-center">
                                        <button onClick={() => toggleAccountRestriction(user.id, 'user')} className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-tighter transition-all active:scale-90 ${user.isRestricted ? 'bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500/20' : 'bg-green-500/10 text-green-500 border border-green-500/20 hover:bg-green-500/20'}`}>
                                            {user.isRestricted ? 'Locked' : 'Open'}
                                        </button>
                                    </td>
                                    <td className="p-4 text-right">
                                        <button className="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-white active:scale-90 transition-transform">Profile</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                                    ...
                                    <div className="flex flex-col items-center gap-3 w-full">
                                        ...
                                        {isEditing && (
                                            <button 
                                                ...
                                                className="text-[9px] font-black text-slate-500 hover:text-white uppercase tracking-widest underline active:opacity-50"
                                            >
                                                Cancel Edit
                                            </button>
                                        )}
                            ...
                            <div className="flex gap-2">
                                {game.winningNumber && !game.payoutsApproved && (
                                    <button 
                                        ...
                                        className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white font-black py-3 rounded-xl text-[10px] uppercase tracking-widest transition-all shadow-lg shadow-emerald-900/20 active:scale-95 active:bg-emerald-700"
